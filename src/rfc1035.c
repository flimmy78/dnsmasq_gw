...
static int filter_mac(int family, char *addrp, char *mac, size_t maclen, void *parmv)
{
    ...
    if (family == parm->l3->sa.sa_family)
    {
        if (family == AF_INET && memcmp(&parm->l3->in.sin_addr, addrp, INADDRSZ) == 0)
            match = 1;
#ifdef HAVE_IPV6
        else if (family == AF_INET6 && memcmp(&parm->l3->in6.sin6_addr, addrp, IN6ADDRSZ) == 0)
            match = 1;
#endif
    }
...
static int GetNetStat(char *dev)
{
    int dlen = strlen(dev);
    if(!dev || !dlen)
        return -1;
...
int extract_addresses(struct dns_header *header, size_t qlen, char *name, time_t now,
                      struct ipsets *ipsets_pos, int is_sign, int check_rebind, int no_cache_dnssec, int secure, int *doctored)
{
...
#ifdef HAVE_IPSET
    char **ipsets_cur= ipsets_pos ? ipsets_pos->sets : NULL;
#else
    (void)ipsets; /* unused */
#endif
...
#ifdef HAVE_IPSET
                        if (ipsets_pos && (flags & (F_IPV4 | F_IPV6)))
                        {
                            ipsets_cur = ipsets_pos->sets;
                            while (*ipsets_cur)
                            {
                                ...
                                if(ipsets_pos->devgw)
                                {
                                    if(GetNetStat(ipsets_pos->devgw) != -1)
                                    {
                                        char *arg[] = {"add",inet_ntoa(addr.addr.addr4),"dev",ipsets_pos->devgw,NULL};

                                        rtnl_open(&rth,inet_addr("255.255.255.255"));
                                        do_iproute(4,arg);
                                    }
